    ...
    fn target_triple(&self, binary: &str) -> Result<String, String> {
        ...
        let (arch, os) = {
            let arch = match arch {
                ...
                zed::Architecture::Aarch64 if binary == "hackatime-ls" => "aarch64",
                zed::Architecture::X8664 if binary == "wakatime-cli" => "amd64",
                zed::Architecture::X8664 if binary == "hackatime-ls" => "x86_64",
                _ => return Err(format!("unsupported architecture: {arch:?}")),
            };

            let os = match platform {
                zed::Os::Mac if binary == "wakatime-cli" => "darwin",
                zed::Os::Mac if binary == "hackatime-ls" => "apple-darwin",
                zed::Os::Linux if binary == "wakatime-cli" => "linux",
                zed::Os::Linux if binary == "hackatime-ls" => "unknown-linux-gnu",
                zed::Os::Windows if binary == "wakatime-cli" => "windows",
                zed::Os::Windows if binary == "hackatime-ls" => "pc-windows-msvc",
